...
__m256i sppm_pixel_data_lookup_hash_l(PixelDataLookup *lookup, __m256i x, __m256i y, __m256i z) {
    __m256i x_m = _mm256_mul_epi32(x, _mm256_set1_epi32(18397));
    __m256i y_m = _mm256_mul_epi32(y, _mm256_set1_epi32(20483));
    __m256i z_m = _mm256_mul_epi32(z, _mm256_set1_epi32(29303));
...
void sppm_pixel_data_lookup_to_grid_l(PixelDataLookup *lookup, __m256 loc_x, __m256 loc_y, __m256 loc_z, __m256* ind_x, __m256* ind_y, __m256* ind_z) {
    ...
    *ind_x = _mm256_floor_ps(_mm256_max_ps(_mm256_set1_ps(0.0f), _mm256_mul_ps(from_min_x, inv_grid_res)));

    __m256 grid_min_y = _mm256_set1_ps(lookup->grid_min.y);
    __m256 from_min_y = _mm256_sub_ps(loc_y, grid_min_y);
    *ind_y = _mm256_floor_ps(_mm256_max_ps(_mm256_set1_ps(0.0f), _mm256_mul_ps(from_min_y, inv_grid_res)));

    __m256 grid_min_z = _mm256_set1_ps(lookup->grid_min.z);
    __m256 from_min_z = _mm256_sub_ps(loc_z, grid_min_z);
    *ind_z = _mm256_floor_ps(_mm256_max_ps(_mm256_set1_ps(0.0f), _mm256_mul_ps(from_min_z, inv_grid_res)));
}
...
void sppm_build_pixel_data_lookup(PixelDataLookup *lookup, PixelData *pixel_datas) {
    ...
    float max_radius;
    float store_impl[NUM_FLOAT_SIMD];
...
//    TODO: collect and condense the branch_cache to skip operations
...
void sppm_camera_pass(SPPM *sppm, PixelData *pixel_datas) {
    ...
    for(i = 0, width_tracker = 8; i < pixel_datas->size_float_simd; i += NUM_FLOAT_SIMD, width_tracker += NUM_FLOAT_SIMD) {
        ...
        __m256 vp_attenuation_x = _mm256_set1_ps(0.0f);
        __m256 vp_attenuation_y = _mm256_set1_ps(0.0f);
        __m256 vp_attenuation_z = _mm256_set1_ps(0.0f);

        __m256 direct_radiance_x = _mm256_set1_ps(0.0f);
        __m256 direct_radiance_y = _mm256_set1_ps(0.0f);
        __m256 direct_radiance_z = _mm256_set1_ps(0.0f);

        __m256 not_completion_vector = _mm256_set1_ps(1.0f);
        for (int c_depth = 0; c_depth < sppm->ray_max_depth; c_depth++) {
            ...
            to_store_isect.mesh_material.data = _mm256_blendv_ps(to_store_isect.mesh_material.data,
                                                                 temp_isect.mesh_material.data, not_completion_vector);
            ...
            not_completion_vector = _mm256_blendv_ps( not_completion_vector, _mm256_set1_ps(0.0f),cur_selected);
        ...
        _mm256_store_ps(&pixel_datas->direct_radiance.x[i], direct_radiance_x);
        _mm256_store_ps(&pixel_datas->direct_radiance.y[i], direct_radiance_y);
        _mm256_store_ps(&pixel_datas->direct_radiance.z[i], direct_radiance_z);
        ...
        if(width_tracker > W) {
            switch (i % W) {
                case 1: {
                    ...
                    y = _mm256_blend_ps(y, t1, 0b00000001);
                    width_tracker = 1;
                    break;}
                case 2: {
                ...
                case 3: {
                    ...
                    y = _mm256_blend_ps(y, t1, 0b00000111);
                    width_tracker = 3;
                    break;}
                case 4: {
...
void sppm_photon_pass(SPPM *sppm, PixelDataLookup *lookup, PixelData *pixel_datas) {
    ...
    for (i = 0; i < (sppm->num_photons / NUM_FLOAT_SIMD) * NUM_FLOAT_SIMD; i += NUM_FLOAT_SIMD) {
        ...
        __m256 pdf_emitter = _mm256_set1_ps(sppm->scene->accum_probabilities[0]);
...
//      only spheres
        ...
        __m256 pdf_pos = _mm256_div_ps(_mm256_set1_ps(1.0f),
        ...
        __m256 t0 = _mm256_mul_ps(pdf_emitter, _mm256_mul_ps(pdf_pos, pdf_dir));
        __m256 light_radiance_x = _mm256_div_ps(emission_x, t0);
        __m256 light_radiance_y = _mm256_div_ps(emission_y, t0);
        __m256 light_radiance_z = _mm256_div_ps(emission_z, t0);

        IntersectionM temp_isect;
        for (int c_depth = 0; c_depth < sppm->ray_max_depth; c_depth++) {
            __m256 do_intersect = scene_intersect_m(sppm->scene, ray_o_x, ray_o_y, ray_o_z, ray_d_x, ray_d_y, ray_d_z,
                                                    &ray_t_max, &temp_isect);
            not_completion_vector = _mm256_blendv_ps(_mm256_set1_ps(0.0f), not_completion_vector,
            ...
            if (c_depth > 0) {
                ...
                float isect_p_x_impl[8];
                float isect_p_y_impl[8];
                float isect_p_z_impl[8];
                ...
                for (int j = 0; j < NUM_FLOAT_SIMD; j++) {
                    ...
                    __m256 cur_neg_ray_d_x = _mm256_neg_ps(_mm256_permutevar8x32_ps(ray_d_x, shufmask));
                    __m256 cur_neg_ray_d_y = _mm256_neg_ps(_mm256_permutevar8x32_ps(ray_d_y, shufmask));
                    __m256 cur_neg_ray_d_z = _mm256_neg_ps(_mm256_permutevar8x32_ps(ray_d_z, shufmask));

                    shufmask = _mm256_add_epi32(shufmask, _mm256_set1_epi32(1));
                    for (int cur_arr_ind = 0; cur_arr_ind < (lookup->hash_table[ht_loc_impl[j]].size / NUM_FLOAT_SIMD) *
                                                            NUM_FLOAT_SIMD; cur_arr_ind += NUM_FLOAT_SIMD) {
                        __m256i pd_index = _mm256_load_si256((__m256i *)&lookup->hash_table[ht_loc_impl[j]]);
...
//                      conscious choice to not store cur_vp_intersection->wo, since it is not needed
                        ...
                        __m256 t2 = vector3fl_dot(cur_vp_intersection_n_x, cur_vp_intersection_n_y,
                                                  ...
                                                  cur_neg_ray_d_z);
                        __m256 cmp1 = _mm256_and_ps(_mm256_cmp_ps(t1, _mm256_set1_ps(0.0f), _CMP_LT_OQ),
                                                    _mm256_cmp_ps(t2, _mm256_set1_ps(0.0f), _CMP_GT_OQ));
                        ...
                        __m256 bsdf_x = _mm256_blendv_ps(_mm256_set1_ps(0.0f),
                                                         _mm256_mul_ps(cur_vp_intersection_mesh_albedo_x, inv_pi),
                                                         cmp1);
                        __m256 bsdf_y = _mm256_blendv_ps(_mm256_set1_ps(0.0f),
                                                         _mm256_mul_ps(cur_vp_intersection_mesh_albedo_y, inv_pi),
                                                         cmp1);
                        __m256 bsdf_z = _mm256_blendv_ps(_mm256_set1_ps(0.0f),
                                                         _mm256_mul_ps(cur_vp_intersection_mesh_albedo_z, inv_pi),
                                                         cmp1);

                        bsdf_x = _mm256_mul_ps(bsdf_x, light_radiance_x);
                        bsdf_y = _mm256_mul_ps(bsdf_y, light_radiance_y);
                        bsdf_z = _mm256_mul_ps(bsdf_z, light_radiance_z);

                        _mm256_add_scatter_1_ps(_mm256_mul_ps(bsdf_x, cmp0), pixel_datas->cur_flux.x, pd_index);
                        _mm256_add_scatter_1_ps(_mm256_mul_ps(bsdf_y, cmp0), pixel_datas->cur_flux.y, pd_index);
                        _mm256_add_scatter_1_ps(_mm256_mul_ps(bsdf_z, cmp0), pixel_datas->cur_flux.z, pd_index);
                        _mm256_add_scatter_1_epi32(_mm256_mul_ps(_mm256_set1_epi32(1),  cmp0), pixel_datas->cur_photons.data, pd_index);
                    }
                }
                __m256 cur_attenuation_x, cur_attenuation_y, cur_attenuation_z;
                bsdf_sample_m(&temp_isect, &cur_attenuation_x, &cur_attenuation_y, &cur_attenuation_z);
    ...
    for (; i < sppm->num_photons; i++) {
        ...
        for (int c_depth = 0; c_depth < sppm->ray_max_depth; c_depth++) {
            ...
            if (c_depth > 0) {  // Direct illumination is accounted for in the camera pass
                ...
                int ht_loc = sppm_pixel_data_lookup_hash(lookup, loc_x, loc_y, loc_z);
                for (int cur_arr_ind = 0; cur_arr_ind < (lookup->hash_table[ht_loc].size / NUM_FLOAT_SIMD) * NUM_FLOAT_SIMD; cur_arr_ind += NUM_FLOAT_SIMD) {
                    __m256i pd_index = _mm256_load_si256((__m256i *)&lookup->hash_table[ht_loc]);
                    __m256 radius = _mm256_i32gather_ps(&pixel_datas->radius.data, pd_index, sizeof(float));
                    __m256 cur_vp_intersection_x = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.p.x, pd_index, sizeof(float));
                    __m256 cur_vp_intersection_y = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.p.y, pd_index, sizeof(float));
                    __m256 cur_vp_intersection_z = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.p.z, pd_index, sizeof(float));
                    ...
                    __m256 cur_vp_intersection_n_x = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.n.x, pd_index, sizeof(float));
                    __m256 cur_vp_intersection_n_y = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.n.y, pd_index, sizeof(float));
                    __m256 cur_vp_intersection_n_z = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.n.z, pd_index, sizeof(float));

                    __m256 cur_vp_intersection_wi_x = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.wi.x, pd_index, sizeof(float));
                    __m256 cur_vp_intersection_wi_y = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.wi.y, pd_index, sizeof(float));
                    __m256 cur_vp_intersection_wi_z = _mm256_i32gather_ps(&pixel_datas->cur_vp_intersection.wi.z, pd_index, sizeof(float));
...
void sppm_consolidate(PixelData *pixel_datas, float alpha) {
    ...
    for (i = 0; i < pixel_datas->size_float_simd; i += NUM_FLOAT_SIMD) {
        ...
        __m256i cur_photons = _mm256_load_si256((__m256i *)&pixel_datas->cur_photons.data[i]);
        __m256i num_photons = _mm256_load_si256((__m256i *)&pixel_datas->num_photons.data[i]);

        __m256 imask = _mm256_cmpgt_epi32(cur_photons, _mm256_set1_epi32(0));
        __m256 mask = _mm256_cvtepi32_ps(imask);

        __m256 new_num_photons = _mm256_add_ps(num_photons, _mm256_mul_ps(alpha_vector, cur_photons));
        ...
        _mm256_store_si256((__m256i *)&pixel_datas->num_photons.data[i], _mm256_blendv_epi8(num_photons, new_num_photons, mask));
        _mm256_store_ps(&pixel_datas->radius.data[i], _mm256_blendv_ps(radius, new_radius, mask));

        _mm256_store_si256((__m256i *)&pixel_datas->cur_photons.data[i], _mm256_set1_epi32(0));
        _mm256_store_ps(&pixel_datas->cur_flux.x[i], _mm256_set1_ps(0.0f));
        _mm256_store_ps(&pixel_datas->cur_flux.y[i], _mm256_set1_ps(0.0f));
        _mm256_store_ps(&pixel_datas->cur_flux.z[i], _mm256_set1_ps(0.0f));
    }
    for (; i < pixel_datas->size; i++) {
        ...
        int cur_photons = pixel_datas->cur_photons.data[i];
        int num_photons = pixel_datas->num_photons.data[i];
        if (cur_photons > 0) {
